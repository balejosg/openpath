...
function firstDataPayload(writes: string[]): string {
...
await describe('SSE Hub', async () => {
  await test('publishGroupChangedLocal targets only matching group', () => {
    ...
    const parsed = JSON.parse(firstDataPayload(writesA)) as { event?: string; groupId?: string };
  ...
  await test('publishClassroomChangedLocal updates group index and emits new groupId', async () => {
    ...
    const change = JSON.parse(firstDataPayload(writes)) as { event?: string; groupId?: string };
