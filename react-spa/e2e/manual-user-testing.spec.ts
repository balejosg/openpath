/**
 * Manual User Testing Script - Instancia Desplegada
 *
 ...
 * npx playwright test e2e/manual-user-testing.spec.ts --headed --project=chromium
 *
 * Nota: Este spec NO registra tests por defecto. Para ejecutarlo:
 * RUN_MANUAL_E2E_TESTS=1 MANUAL_E2E_TARGET_URL=https://tu-instancia.example.com \
 *   npx playwright test e2e/manual-user-testing.spec.ts --headed --project=chromium
 */
...
const RUN_MANUAL_E2E_TESTS = process.env.RUN_MANUAL_E2E_TESTS === '1';
const TARGET_URL = process.env.MANUAL_E2E_TARGET_URL || '';
...
async function captureScreenshot(page: Page, name: string): Promise<string> {
  ...
  const filename = `manual-e2e-${name}-${timestamp}.png`;
...
function saveErrorReport(): void {
  const reportPath = path.join(
    __dirname,
    'test-results',
    `manual-e2e-errors-${new Date().toISOString().replace(/[:.]/g, '-')}.json`
  );
...
if (RUN_MANUAL_E2E_TESTS) {
  if (!TARGET_URL) {
    throw new Error('MANUAL_E2E_TARGET_URL es obligatorio cuando RUN_MANUAL_E2E_TESTS=1.');
  }

  test.describe('Pruebas Manuales de Usuario - Instancia Desplegada', () => {
    ...
    test('TC02: Exploración del formulario de Login', async () => {
      ...
      if (loginButtonExists) {
        ...
        const errorMessage = page.locator(
          '[role="alert"], .error, .text-red-500, [class*="error"]'
        );
    ...
    test('TC03: Navegación al formulario de registro', async () => {
      ...
      const registerNameInput = page
        .locator(
          '[data-testid="register-name"], input[name="name"], input[placeholder*="nombre" i]'
        )
        .first();
    ...
    test('TC04: Validación del formulario de registro', async () => {
      ...
      if (await emailInput.isVisible().catch(() => false)) {
        ...
        console.log(
          `   ✉️ Validación email inválido: ${hasEmailError ? 'Mostrada' : 'No mostrada'}`
        );
    ...
    test('TC10: Generación de resumen final', async () => {
      ...
      console.log('==========================================');
      console.log(`URL probada: ${TARGET_URL}`);
