...
$defaultSnapshotPath = if ($env:RUNNER_TEMP) {
    ...
    Join-Path $PSScriptRoot 'openpath-windows-process-snapshot.json'
}
...
switch ($Mode) {
    'capture' {
        ...
        Write-Host ("Captured Windows process snapshot with {0} processes at {1}" -f $snapshot.Count, $SnapshotPath)
        break
    }
    ...
    'report' {
        ...
        $interestingProcesses = @(
            $currentProcesses |
                Where-Object {
                    $interestingNames.Contains([string]$_.Name) -and
                    -not $protectedIds.Contains([int]$_.ProcessId)
                } |
                Sort-Object Name, ProcessId
        )
